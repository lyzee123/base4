...
func YoutubeDL(uri string) (YoutubeInfos, error) {
  if !IsYoutubeURL(uri) {
    return YoutubeInfos{}, errors.New("Url Invalid")
  }
  params := url.Values{}
  params.Add("q", uri)
  params.Add("vt", "home")
  ...
  for _, dat := range data["links"].(map[string]interface{})["mp3"].(map[string]interface{}) {
    ...
    link.Audio = append(link.Audio, YoutubeAV{
      Size:    a["size"].(string),
      Format:  a["f"].(string),
      Quality: a["q"].(string),
      Url: func() (string, error) {
  ...
  for _, dat := range data["links"].(map[string]interface{})["mp4"].(map[string]interface{}) {
    ...
    link.Video = append(link.Video, YoutubeAV{
      Size:    a["size"].(string),
      Format:  a["f"].(string),
      Quality: a["q"].(string),
      Url: func() (string, error) {
